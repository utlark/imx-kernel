 ...
 */
 ...
 static unsigned char mchpar1xxx_parameters [0x14] = {
	0x18, 0x02, 0x80, 0x04, 0x04, 0x08, 0x08, 0x08, 0x03, 0x00, 0x00, 0x02, 0xB1, 0x89, 0x40, // default
	0x60, // PenStateReportDelay
	0x00, // default
	0x60, // TouchReportDelay
	0x80, 0x80 // default
};
...
static int mchpar1xxx_connect(struct serio *serio, struct serio_driver *drv)
{
	...
	serio_write(serio, 0x55);
	serio_write(serio, 1);
	serio_write(serio, 0x13);
	usleep_range(10000, 20000);

	serio_write(serio, 0x55);
	serio_write(serio, 4 + 0x14);
	serio_write(serio, 0x21);
	serio_write(serio, 0);
	serio_write(serio, 0); 
	serio_write(serio, 0x14);
	int i;
	for (i = 0; i < 0x14; i ++)
	{
		serio_write(serio, mchpar1xxx_parameters[i]);
	}
	usleep_range(10000, 20000);

	serio_write(serio, 0x55);
	serio_write(serio, 1);
	serio_write(serio, 0x12);
	usleep_range(10000, 20000);
